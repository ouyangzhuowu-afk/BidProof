...
function showAuth(setup, message = '') {
  ...
  if (setup) authTrialMode = false;
  const trialEnabled = !setup && Boolean(authStatus?.trial_join_enabled);
  document.querySelector('#auth-mode-wrap').hidden = !trialEnabled;
  if (!trialEnabled) authTrialMode = false;
...
function setAuthMode(trial) {
  authTrialMode = Boolean(trial) && Boolean(authStatus?.trial_join_enabled) && !authSetupRequired;
  applyAuthMode('');
  setTimeout(() => document.querySelector(authTrialMode ? '#auth-join-code' : '#auth-username')?.focus(), 0);
  refreshIcons();
}

function applyAuthMode(message = '') {
  const setup = authSetupRequired;
  const trial = !setup && authTrialMode;
...
async function createMember(event) {
  ...
  const direct = memberCreateMode === 'direct';
...
function setMemberCreateMode(mode) {
  memberCreateMode = mode === 'direct' ? 'direct' : 'invite';
  document.querySelectorAll('[data-member-mode]').forEach((button) => {
    button.classList.toggle('is-active', button.dataset.memberMode === memberCreateMode);
  });
  const direct = memberCreateMode === 'direct';
