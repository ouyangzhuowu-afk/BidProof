...
function raw(value) {
  return { __safe: true, value: String(value ?? ""), toString() {
    return this.value;
  } };
}
...
openIntakeButtons.forEach((selector) => document.querySelector(selector).addEventListener("click", () => {
...
document.querySelector("#run-project-filter").addEventListener("change", (event) => {
...
document.querySelector("#run-tag-filter").addEventListener("change", (event) => {
  store.runTagFilter = event.target.value.trim();
  store.selectedRunIds.clear();
  loadRuns();
});
document.querySelector("#run-assignee-filter").addEventListener("change", (event) => {
  store.runAssigneeFilter = event.target.value;
  store.selectedRunIds.clear();
  loadRuns();
});
document.querySelector("#run-reviewer-filter").addEventListener("change", (event) => {
  store.runReviewerFilter = event.target.value;
  store.selectedRunIds.clear();
  loadRuns();
});
document.querySelector("#run-favorite-filter").addEventListener("change", (event) => {
...
document.querySelector("#rescan-run").addEventListener("click", () => {
  var _a;
  store.rescanParentId = ((_a = store.currentRun) == null ? void 0 : _a.run_id) || null;
  openIntake();
});
...
function renderCurrentUser() {
  ...
  document.querySelector("#current-username").textContent = ((_a = store.currentUser) == null ? void 0 : _a.username) || "";
  document.querySelector("#current-user-role").textContent = store.currentUser ? roleLabel(store.currentUser.role) : "";
  document.querySelector("#password-username").value = ((_b = store.currentUser) == null ? void 0 : _b.username) || "";
}
...
async function openIntake() {
  var _a;
  await loadProjects();
  if (store.rescanParentId && ((_a = store.currentRun) == null ? void 0 : _a.project_id)) document.querySelector("#tender-project").value = store.currentRun.project_id;
...
async function loadRuns() {
  ...
  try {
    ...
    store.selectedRunIds.forEach((id) => {
      if (!visibleRuns.some((run) => run.run_id === id)) store.selectedRunIds.delete(id);
    });
...
async function loadNotifications() {
  ...
  try {
    ...
    setHtml(target, html`<div class="empty-state error-text">${error.message}，提醒加载失败。</div>`);
  }
}
function renderOverview(runs) {
  ...
  const decided = runs.filter((run) => {
    var _a;
    return (_a = run.decision) == null ? void 0 : _a.decision;
  }).length;
  const items = [
...
function renderRunRow(run) {
  ...
  const decision = ((_a = run.decision) == null ? void 0 : _a.decision) || "未记录";
  ...
  row.querySelector("[data-run-select]").addEventListener("change", (event) => {
    if (event.target.checked) store.selectedRunIds.add(run.run_id);
    else store.selectedRunIds.delete(run.run_id);
    updateBulkControls();
  });
...
function toggleAllRuns(event) {
  document.querySelectorAll("[data-run-select]").forEach((input) => {
    ...
    if (event.target.checked) store.selectedRunIds.add(input.dataset.runSelect);
    else store.selectedRunIds.delete(input.dataset.runSelect);
  });
  updateBulkControls();
}
...
async function loadMembers() {
  ...
  try {
    ...
    const canManage = ["OWNER", "ADMIN"].includes((_a = store.currentUser) == null ? void 0 : _a.role);
    ...
    setHtml(target, html`<div class="empty-state error-text">${error.message}，成员加载失败。</div>`);
  }
  refreshIcons();
}
...
async function loadOperations() {
  ...
  try {
    ...
    setHtml(healthTarget, html`<div class="health-grid"><span><small>数据库</small><strong>${health.database || "unknown"}</strong></span><span><small>备份</small><strong>${backupStatusLabel(health.backup_status)}</strong></span><span><small>失败作业</small><strong>${Number(health.failed_jobs || 0)}</strong></span><span><small>最近验证</small><strong>${health.last_verified_backup_at ? formatDate(health.last_verified_backup_at) : "无"}</strong></span></div><small class="health-reasons">${((_a = health.degraded_reasons) == null ? void 0 : _a.length) ? `降级原因：${health.degraded_reasons.join("、")}` : "未发现降级原因"}</small>`);
    ...
    setHtml(healthTarget, html`<div class="empty-state error-text">${error.message}，运行状态加载失败。</div>`);
  }
  try {
  ...
  document.querySelector("#purge-retention").hidden = !["OWNER", "ADMIN"].includes((_b = store.currentUser) == null ? void 0 : _b.role);
  refreshIcons();
}
...
async function loadMfaStatus() {
  ...
  try {
    ...
    setHtml(status, payload.mfa_enabled ? html`<strong>已启用。</strong><span>登录时需要验证器或恢复码。</span>` : html`<span>尚未启用二次验证。</span>`);
  } catch (error) {
...
async function loadApiTokens() {
  ...
  try {
    ...
    setHtml(target, payload.tokens.length ? payload.tokens.map((token) => html`<article class="backup-row"><span class="operation-main"><strong>${token.name}</strong><small>${token.token_prefix} · ${token.revoked_at ? "已撤销" : "有效"}</small></span>${token.revoked_at ? "" : html`<button class="button secondary" type="button" data-revoke-token="${token.token_id}">撤销</button>`}</article>`) : html`<div class="empty-state">还没有 API 令牌。</div>`);
...
function showDecision() {
  ...
  select.replaceChildren(...unresolved.map((item) => {
    ...
    option.selected = ((_b2 = (_a2 = store.currentRun.decision) == null ? void 0 : _a2.unresolved_requirement_ids) == null ? void 0 : _b2.includes(item.requirement_id)) || false;
    return option;
  }));
  const decision = ((_a = store.currentRun.decision) == null ? void 0 : _a.decision) || "HOLD";
  const radio = document.querySelector(`#decision-form input[value="${decision}"]`);
  if (radio) radio.checked = true;
  document.querySelector("#decision-note").value = ((_b = store.currentRun.decision) == null ? void 0 : _b.note) || "";
  setHtml(document.querySelector("#decision-context-content"), html`<div class="context-metric"><strong>${store.currentRun.blocker_count}</strong><span>资格 / 废标风险</span></div><div class="context-metric"><strong>${store.currentRun.unresolved_count}</strong><span>未解决要求</span></div><div class="context-metric"><strong>${store.currentRun.requirement_count}</strong><span>全部要求项</span></div><p>${((_c = store.currentRun.decision) == null ? void 0 : _c.note) || "尚未记录人工决定。请先核对高风险项与证据缺口。"}</p>`);
  refreshIcons();
}
...
function renderDetail() {
  ...
  const metrics = [
    ...
    ["人工决定", decisionLabel(((_a = store.currentRun.decision) == null ? void 0 : _a.decision) || "未记录"), "decision", "clipboard-check"]
  ];
...
function renderSourceFiles() {
  ...
  const documents = ((_a = store.currentRun) == null ? void 0 : _a.source_documents) || [];
  if (!documents.length) {
...
async function loadVersionDiff() {
  ...
  if (!((_a = store.currentRun) == null ? void 0 : _a.parent_run_id)) {
    section.hidden = true;
    return;
  }
...
async function loadRemediationOwners() {
  ...
  setHtml(requirementSelect, [html`<option value="">不关联具体要求</option>`, ...(((_a = store.currentRun) == null ? void 0 : _a.requirements) || []).map((item) => html`<option value="${item.requirement_id}">${item.requirement_id} · ${item.label}</option>`)]);
}
...
function roleLabel(value) {
  return { OWNER: "所有者", ADMIN: "管理员", REVIEWER: "复核人", VIEWER: "只读成员" }[value] || value;
}
function jobStatusLabel(value) {
  return { PENDING: "排队中", RUNNING: "扫描中", COMPLETED: "已完成", FAILED: "失败", CANCELLED: "已取消" }[value] || value;
}
function backupStatusLabel(value) {
  return { verified: "已验证", unverified: "未验证", missing: "无备份" }[value] || value || "未知";
}
function renderRisks() {
  const risks = store.currentRun.requirements.filter((item) => ["FATAL", "QUALIFICATION", "DEADLINE"].includes(item.category) && item.status !== "PASS").sort((a, b) => riskRank(a) - riskRank(b)).slice(0, 4);
...
function renderMatrix() {
  ...
  const filtered = store.currentRun.requirements.filter((item) => {
    ...
    const haystack = `${item.category} ${item.label} ${item.title} ${((_a = item.source) == null ? void 0 : _a.quote) || ""}`.toLocaleLowerCase("zh-CN");
...
function citationMarkup(item) {
  ...
  const evidenceMarkup = evidence.length ? evidence.map((entry) => html`<strong>${entry.filename} · ${locatorLabel(entry)}</strong><p>${entry.quote || "已定位"}</p>`) : html`<strong class="muted">未匹配，需人工复核</strong>`;
...
function locatorLabel(item) {
  var _a;
  return ((_a = item == null ? void 0 : item.locator) == null ? void 0 : _a.label) || "定位缺失";
}
...
function requirementFooter(item) {
  const actions = item.status === "PASS" || item.status === "FAIL" ? html`<button class="mini-button" data-review="CONFIRM" data-id="${item.requirement_id}">确认结论</button><button class="mini-button" data-review="REJECT" data-id="${item.requirement_id}">驳回结论</button>` : html`<button class="mini-button" data-review="REQUEST_EVIDENCE" data-id="${item.requirement_id}">请求证据</button><button class="mini-button" data-review="CONFIRM" data-id="${item.requirement_id}">保留复核</button>`;
...
async function loadAccuracySummary() {
  ...
  try {
    ...
    setHtml(target, html`<div class="empty-state error-text">指标加载失败</div>`);
  }
}
