...
def auth_status(request: Request) -> dict:
    users = count_users()
    trial_join_enabled = bool(TRIAL_JOIN_CODE) and users > 0
    if users == 0:
        return {
            "setup_required": True,
            "authenticated": False,
...
@app.post("/api/auth/trial-join", status_code=201)
def trial_join(request: Request, response: Response, payload: TrialJoinRequest) -> dict:
    ...
    if not TRIAL_JOIN_CODE:
        raise HTTPException(status_code=403, detail="试用加入未开放")
    now = time.monotonic()
    attempt_key = _login_attempt_key(request, f"trial:{payload.username}")
    _enforce_login_rate_limit(attempt_key, now)
    if len(payload.join_code.strip()) != len(TRIAL_JOIN_CODE) or not secrets.compare_digest(
        payload.join_code.strip(), TRIAL_JOIN_CODE
    ):
        _record_failed_login(attempt_key, now)
    ...
    try:
        user = create_user(workspace_id, username, _password_hash(payload.password), "REVIEWER")
    except sqlite3.IntegrityError as exc:
        raise HTTPException(status_code=409, detail="用户名已存在，请直接登录") from exc
    ensure_workspace(workspace_id, user["user_id"], "REVIEWER")
    _login_attempts.pop(attempt_key, None)
    _issue_session(response, user["user_id"], request.url.scheme == "https" or request.headers.get("X-Forwarded-Proto") == "https")
    record_audit_event(workspace_id, user["user_id"], "AUTH_TRIAL_JOINED", None, {"username": user["username"], "role": "REVIEWER"})
    return {"user_id": user["user_id"], "workspace_id": workspace_id, "username": user["username"], "role": "REVIEWER"}
