...
let runSearch = '';
let runTagFilter = '';
let runAssigneeFilter = '';
let runReviewerFilter = '';
let runFavoriteOnly = false;
let runSort = 'updated_desc';
...
openIntakeButtons.forEach((selector) => document.querySelector(selector).addEventListener('click', () => { rescanParentId = null; openIntake(); }));
...
document.querySelector('#run-scope').addEventListener('change', (event) => { runScope = event.target.value; selectedRunIds.clear(); loadRuns(); });
document.querySelector('#run-project-filter').addEventListener('change', (event) => { projectFilter = event.target.value; selectedRunIds.clear(); loadRuns(); });
document.querySelector('#run-search').addEventListener('input', (event) => {
  ...
  runSearchTimer = setTimeout(() => { selectedRunIds.clear(); loadRuns(); }, 250);
});
document.querySelector('#run-tag-filter').addEventListener('change', (event) => { runTagFilter = event.target.value.trim(); selectedRunIds.clear(); loadRuns(); });
document.querySelector('#run-assignee-filter').addEventListener('change', (event) => { runAssigneeFilter = event.target.value; selectedRunIds.clear(); loadRuns(); });
document.querySelector('#run-reviewer-filter').addEventListener('change', (event) => { runReviewerFilter = event.target.value; selectedRunIds.clear(); loadRuns(); });
document.querySelector('#run-favorite-filter').addEventListener('change', (event) => { runFavoriteOnly = event.target.checked; selectedRunIds.clear(); loadRuns(); });
document.querySelector('#run-sort').addEventListener('change', (event) => { runSort = event.target.value; selectedRunIds.clear(); loadRuns(); });
...
document.querySelector('#rescan-run').addEventListener('click', () => { rescanParentId = currentRun?.run_id || null; openIntake(); });
...
document.querySelector('#requirement-search').addEventListener('input', (event) => {
  searchTerm = event.target.value.trim().toLocaleLowerCase('zh-CN');
  matrixPage = 1;
  renderMatrix();
});
...
function showAuth(setup, message = '') {
  ...
  document.querySelector('#auth-title').textContent = setup ? '初始化企业管理员' : '登录企业空间';
  document.querySelector('#auth-subtitle').textContent = setup ? '使用运维令牌创建首个企业空间和所有者账号。' : '使用企业账号进入任务与证据数据。';
  document.querySelector('#setup-fields').hidden = !setup;
  document.querySelector('#auth-workspace').required = setup;
  const tokenRequired = setup && Boolean(authStatus?.bootstrap_token_required);
  ...
  document.querySelector('#auth-confirm-wrap').hidden = !setup;
  document.querySelector('#auth-password-confirm').required = setup;
  document.querySelector('#auth-password').minLength = setup ? 12 : 1;
  document.querySelector('#auth-password').autocomplete = setup ? 'new-password' : 'current-password';
  document.querySelector('#auth-password-hint').hidden = !setup;
  document.querySelector('#auth-help').hidden = setup;
  document.querySelector('#auth-submit-label').textContent = setup ? '创建并进入' : '登录';
  ...
  setTimeout(() => document.querySelector(setup ? '#auth-workspace' : '#auth-username').focus(), 0);
  refreshIcons();
}
...
async function submitAuth(event) {
  ...
  setButtonLoading(button, true, authSetupRequired ? '正在初始化' : '正在登录');
  try {
    ...
    currentUser = await request(authSetupRequired ? '/api/auth/bootstrap' : '/api/auth/login', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
    authDialog.close();
    form.reset();
    renderCurrentUser();
    await loadProjects();
...
function renderCurrentUser() {
  ...
  container.hidden = !currentUser;
  document.querySelector('#logout-button').hidden = !currentUser;
  document.querySelector('#current-username').textContent = currentUser?.username || '';
  document.querySelector('#current-user-role').textContent = currentUser ? roleLabel(currentUser.role) : '';
  document.querySelector('#password-username').value = currentUser?.username || '';
}
...
async function submitAccountAction(event) {
  ...
  try {
    const endpoint = accountAction.action === 'INVITE' ? '/api/auth/activate' : '/api/auth/reset-password';
    currentUser = await request(endpoint, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ token: accountAction.token, password }) });
...
async function openIntake() {
  await loadProjects();
  if (rescanParentId && currentRun?.project_id) document.querySelector('#tender-project').value = currentRun.project_id;
...
async function loadRuns() {
  ...
  list.innerHTML = '<div class="run-skeleton"></div><div class="run-skeleton"></div><div class="run-skeleton"></div>';
  refresh.disabled = true;
  try {
    await loadTaskFilterOptions();
    const params = new URLSearchParams({ include_archived: String(runScope !== 'ACTIVE'), sort: runSort });
    if (projectFilter) params.set('project_id', projectFilter);
    if (runSearch) params.set('search', runSearch);
    if (runTagFilter) params.set('tag', runTagFilter);
    if (runAssigneeFilter) params.set('assignee_id', runAssigneeFilter);
    if (runReviewerFilter) params.set('reviewer_id', runReviewerFilter);
    if (runFavoriteOnly) params.set('favorite', 'true');
    const runs = await request(`/api/runs?${params.toString()}`);
    const visibleRuns = runScope === 'ARCHIVED' ? runs.filter((run) => run.archived_at) : runScope === 'ACTIVE' ? runs.filter((run) => !run.archived_at) : runs;
    renderOverview(visibleRuns);
    loadAccuracySummary();
    loadNotifications();
    selectedRunIds.forEach((id) => { if (!visibleRuns.some((run) => run.run_id === id)) selectedRunIds.delete(id); });
    updateBulkControls(visibleRuns);
    if (!visibleRuns.length) {
      const filtered = runSearch || runTagFilter || runAssigneeFilter || runReviewerFilter || runFavoriteOnly || projectFilter;
      list.innerHTML = filtered
        ? '<div class="empty-state"><span>没有符合当前筛选的任务，请调整条件或清除筛选。</span></div>'
        : '<div class="empty-state"><span>还没有扫描任务，请先上传一份招标文件。</span></div>';
      return;
    }
    list.replaceChildren(...visibleRuns.map(renderRunRow));
    updateBulkControls(visibleRuns);
  } catch (error) {
    list.innerHTML = `<div class="empty-state error-text">${escapeHtml(error.message)}，请检查本地服务后重试。</div>`;
    renderOverview([]);
  } finally {
...
async function loadTaskFilterOptions() {
  if (!membersCache.length) membersCache = (await request('/api/members')).members;
  const options = membersCache.filter((member) => member.active).map((member) => `<option value="${escapeHtml(member.user_id)}">${escapeHtml(member.username)} · ${escapeHtml(roleLabel(member.role))}</option>`).join('');
  const assignee = document.querySelector('#run-assignee-filter');
  const reviewer = document.querySelector('#run-reviewer-filter');
  assignee.innerHTML = '<option value="">全部负责人</option>' + options;
  reviewer.innerHTML = '<option value="">全部复核人</option>' + options;
  assignee.value = runAssigneeFilter;
  reviewer.value = runReviewerFilter;
}
...
function clearRunFilters() {
  ...
  document.querySelector('#run-sort').value = runSort;
...
async function loadNotifications() {
  ...
  try {
    ...
    if (!payload.notifications.length) {
      target.innerHTML = '<div class="empty-state success-text">暂无需要立即处理的提醒。</div>';
      return;
    }
    target.innerHTML = payload.notifications.slice(0, 6).map((item) => `<article class="notification-item ${escapeHtml(item.severity)}"><span class="notification-icon"><i data-lucide="${item.type === 'SCAN_JOB_FAILED' ? 'triangle-alert' : 'calendar-clock'}"></i></span><span><strong>${escapeHtml(item.title)}</strong><small>${escapeHtml(item.message)}${item.run_id ? ` · 任务 ${escapeHtml(item.run_id.slice(0, 12))}` : ''}</small></span></article>`).join('');
    ...
    refreshIcons();
  } catch (error) { target.innerHTML = `<div class="empty-state error-text">${escapeHtml(error.message)}，提醒加载失败。</div>`; }
}
...
function renderOverview(runs) {
  ...
  document.querySelector('#overview-grid').replaceChildren(...items.map(([label, value, note, icon, tone]) => {
    ...
    card.innerHTML = `<div class="overview-top"><span>${label}</span><span class="overview-icon"><i data-lucide="${icon}"></i></span></div><strong>${value}</strong><small>${note}</small>`;
    return card;
  }));
}
...
async function submitScan(event) {
  ...
  try {
    ...
    if (rescanParentId) {
      currentRun = await request(`/api/runs/${encodeURIComponent(rescanParentId)}/rescan`, { method: 'POST', body: formData });
    } else {
      const job = await request('/api/jobs', { method: 'POST', body: formData });
      form.reset();
      closeIntake();
      showToast('扫描任务已进入后台队列。');
      currentRun = await waitForJob(job.job_id);
    }
    form.reset();
    closeIntake();
    rescanParentId = null;
...
function renderRunRow(run) {
  ...
  row.innerHTML = `<label class="run-select" aria-label="选择 ${escapeHtml(run.tender_filename)}"><input type="checkbox" data-run-select="${escapeHtml(run.run_id)}" ${selectedRunIds.has(run.run_id) ? 'checked' : ''}><span></span></label><button class="run-open" type="button" aria-label="打开 ${escapeHtml(run.tender_filename)}，${run.blocker_count} 项高风险，${run.unresolved_count} 项待复核"><span class="run-main"><strong title="${escapeHtml(run.tender_filename)}">${escapeHtml(run.tender_filename)}</strong><small>${escapeHtml(formatDate(run.updated_at || run.created_at))} · ${escapeHtml(run.run_id.slice(0, 12))}</small></span><span class="run-stat danger"><b>${run.blocker_count}</b><span>高风险</span></span><span class="run-stat warning"><b>${run.unresolved_count}</b><span>待复核</span></span><span class="decision-pill ${escapeHtml(decision)}">${escapeHtml(decisionLabel(decision))}</span><span class="run-arrow"><i data-lucide="chevron-right"></i></span></button>`;
  row.querySelector('[data-run-select]').addEventListener('change', (event) => { if (event.target.checked) selectedRunIds.add(run.run_id); else selectedRunIds.delete(run.run_id); updateBulkControls(); });
...
function toggleAllRuns(event) {
  document.querySelectorAll('[data-run-select]').forEach((input) => {
    ...
    if (event.target.checked) selectedRunIds.add(input.dataset.runSelect); else selectedRunIds.delete(input.dataset.runSelect);
  });
  updateBulkControls();
}
...
function updateBulkControls(visibleRuns = []) {
  ...
  document.querySelector('#bulk-archive').disabled = !count || runScope === 'ARCHIVED';
  document.querySelector('#bulk-restore').disabled = !count || runScope === 'ACTIVE';
  document.querySelector('#bulk-restore').hidden = runScope === 'ACTIVE';
...
async function bulkManage(action) {
  ...
  if (action === 'DELETE' && !window.confirm(`确定永久删除已选择的 ${selectedRunIds.size} 个任务及其上传文件吗？此操作不可恢复。`)) return;
  try {
    const result = await request('/api/runs/bulk', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ run_ids: [...selectedRunIds], action }) });
    selectedRunIds.clear();
    await loadRuns();
...
async function bulkExportReports() {
  ...
  try {
    ...
    showToast(`已导出 ${selectedRunIds.size} 个任务的 PDF 报告。`);
  } catch (error) {
...
function exportCurrentRun(format) {
  ...
  const link = document.createElement('a');
  link.href = `/api/runs/${encodeURIComponent(currentRun.run_id)}/report.${format}`;
  link.download = `bidproof-${currentRun.run_id.slice(0, 12)}.${format}`;
  document.body.appendChild(link);
  link.click();
  link.remove();
}
...
async function openRun(runId) {
  try {
    currentRun = await request(`/api/runs/${encodeURIComponent(runId)}`);
    activeCategory = 'ALL';
    searchTerm = '';
    matrixPage = 1;
...
async function loadJobs() {
  ...
  target.innerHTML = '<div class="run-skeleton"></div><div class="run-skeleton"></div>';
  refresh.disabled = true;
  try {
    const payload = await request('/api/jobs?limit=200');
    document.querySelector('#jobs-count').textContent = `${payload.jobs.length} 个作业`;
    if (!payload.jobs.length) {
      target.innerHTML = '<div class="empty-state">还没有后台扫描作业。</div>';
      return;
    }
    target.innerHTML = payload.jobs.map((job) => {
      const retry = job.status === 'FAILED' ? `<button class="mini-button" type="button" data-retry-job="${escapeHtml(job.job_id)}"><i data-lucide="refresh-ccw"></i><span>重试</span></button>` : '';
      const cancel = ['PENDING', 'RUNNING'].includes(job.status) ? `<button class="mini-button danger-action" type="button" data-cancel-job="${escapeHtml(job.job_id)}"><i data-lucide="circle-stop"></i><span>取消</span></button>` : '';
      ...
      return `<article class="job-row"><span class="operation-main"><strong>${escapeHtml(job.job_id.slice(0, 12))}</strong><small>${job.run_id ? `任务 ${escapeHtml(job.run_id.slice(0, 12))}` : escapeHtml(job.error || job.progress_message || '等待生成任务')}</small><span class="job-progress" aria-label="处理进度 ${progressLabel}"><span class="job-progress-track"><span style="width:${progressPercent}%"></span></span><small>${escapeHtml(progressLabel)}</small></span></span><span class="status-pill ${escapeHtml(job.status)}">${escapeHtml(jobStatusLabel(job.status))}</span><span>${Number(job.attempts || 0)}</span><time>${escapeHtml(formatDate(job.updated_at))}</time><span>${retry}${cancel}</span></article>`;
    }).join('');
    ...
    target.innerHTML = `<div class="empty-state error-text">${escapeHtml(error.message)}，作业记录加载失败。</div>`;
  } finally {
    refresh.disabled = false;
    refreshIcons();
  }
...
async function loadMembers() {
  ...
  target.innerHTML = '<div class="run-skeleton"></div>';
  try {
    const payload = await request('/api/members');
    membersCache = payload.members;
    const canManage = ['OWNER', 'ADMIN'].includes(currentUser?.role);
    document.querySelector('#member-form').hidden = !canManage;
    target.innerHTML = membersCache.map((member) => {
      const roleControls = canManage && member.role !== 'OWNER' ? `<select data-member-role="${escapeHtml(member.user_id)}" aria-label="${escapeHtml(member.username)} 的角色"><option value="ADMIN" ${member.role === 'ADMIN' ? 'selected' : ''}>管理员</option><option value="REVIEWER" ${member.role === 'REVIEWER' ? 'selected' : ''}>复核人</option><option value="VIEWER" ${member.role === 'VIEWER' ? 'selected' : ''}>只读成员</option></select><button class="mini-button" type="button" data-member-active="${escapeHtml(member.user_id)}" data-active="${member.active}">${member.active ? '停用' : '启用'}</button>` : `<span class="role-label">${escapeHtml(roleLabel(member.role))}</span>`;
      const resetControl = canManage && member.active ? `<button class="mini-button" type="button" data-member-reset="${escapeHtml(member.user_id)}">重置密码</button>` : '';
      const controls = `${roleControls}${resetControl}`;
      return `<article class="member-row ${member.active ? '' : 'is-inactive'}"><span class="member-avatar"><i data-lucide="user-round"></i></span><span class="operation-main"><strong>${escapeHtml(member.username)}</strong><small>${escapeHtml(member.user_id.slice(0, 12))} · ${member.active ? '可登录' : '已停用'}</small></span><span class="member-controls">${controls}</span></article>`;
  ...
  } catch (error) { target.innerHTML = `<div class="empty-state error-text">${escapeHtml(error.message)}，成员加载失败。</div>`; }
  refreshIcons();
}
...
async function loadProjects() {
  ...
  try {
    ...
    const activeProjects = projectsCache.filter((project) => !project.archived_at);
    const filter = document.querySelector('#run-project-filter');
    filter.innerHTML = '<option value="">全部项目</option>' + projectsCache.map((project) => `<option value="${escapeHtml(project.project_id)}">${escapeHtml(project.code)} · ${escapeHtml(project.name)}${project.archived_at ? '（已归档）' : ''}</option>`).join('');
    ...
    tenderProject.innerHTML = activeProjects.map((project) => `<option value="${escapeHtml(project.project_id)}">${escapeHtml(project.code)} · ${escapeHtml(project.name)}</option>`).join('');
    ...
    target.innerHTML = projectsCache.map((project) => `<article class="project-row ${project.archived_at ? 'is-inactive' : ''}"><span class="member-avatar"><i data-lucide="folder"></i></span><span class="operation-main"><strong>${escapeHtml(project.name)}</strong><small>${escapeHtml(project.code)} · ${project.archived_at ? '已归档' : '可创建扫描'}</small></span>${canManage && project.code !== 'DEFAULT' ? `<button class="mini-button" type="button" data-project-archive="${escapeHtml(project.project_id)}" data-archived="${Boolean(project.archived_at)}">${project.archived_at ? '恢复' : '归档'}</button>` : '<span class="role-label">默认</span>'}</article>`).join('');
    target.querySelectorAll('[data-project-archive]').forEach((button) => button.addEventListener('click', () => toggleProject(button)));
  } catch (error) {
    target.innerHTML = `<div class="empty-state error-text">${escapeHtml(error.message)}，项目加载失败。</div>`;
  }
  refreshIcons();
}
...
async function createMember(event) {
  ...
  try {
    const invitation = await request('/api/auth/invitations', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ username: document.querySelector('#member-username').value.trim(), role: document.querySelector('#member-role').value }) });
    form.reset();
...
function showSecureLink(target, path, prefix) {
  ...
  target.innerHTML = `${escapeHtml(prefix)} <a href="${escapeHtml(url)}">打开链接</a> <button class="text-button" type="button" data-copy-secure-link>复制</button>`;
...
async function loadOperations() {
  ...
  try {
    ...
    document.querySelector('#retention-preview').innerHTML = `<strong>${preview.count}</strong><span>个已归档任务早于 ${escapeHtml(formatDate(preview.cutoff))}，执行前仍会再次预览。</span>`;
    healthTarget.innerHTML = `<div class="health-grid"><span><small>数据库</small><strong>${escapeHtml(health.database || 'unknown')}</strong></span><span><small>备份</small><strong>${escapeHtml(backupStatusLabel(health.backup_status))}</strong></span><span><small>失败作业</small><strong>${Number(health.failed_jobs || 0)}</strong></span><span><small>最近验证</small><strong>${escapeHtml(health.last_verified_backup_at ? formatDate(health.last_verified_backup_at) : '无')}</strong></span></div><small class="health-reasons">${health.degraded_reasons?.length ? `降级原因：${escapeHtml(health.degraded_reasons.join('、'))}` : '未发现降级原因'}</small>`;
    document.querySelector('#workspace-usage').innerHTML = `<div class="health-grid"><span><small>任务</small><strong>${usage.runs}</strong></span><span><small>作业</small><strong>${usage.scan_jobs}</strong></span><span><small>整改项</small><strong>${usage.remediations}</strong></span><span><small>审计事件</small><strong>${usage.audit_events}</strong></span></div>`;
    document.querySelector('#workspace-privacy').innerHTML = `<p>${escapeHtml(privacy.boundary)}</p><p>${escapeHtml(privacy.deletion)}</p><small>当前保留策略：${privacy.retention_days} 天</small>`;
  } catch (error) { healthTarget.innerHTML = `<div class="empty-state error-text">${escapeHtml(error.message)}，运行状态加载失败。</div>`; }
  try {
    const payload = await request('/api/backups');
    backupTarget.innerHTML = payload.backups.length ? payload.backups.map((backup) => `<article class="backup-row"><span class="operation-main"><strong>${escapeHtml(backup.backup_id)}</strong><small>${escapeHtml(formatDate(backup.created_at))}</small></span><span class="status-pill ${backup.valid ? 'COMPLETED' : 'FAILED'}">${backup.valid ? '已验证' : '未验证'}</span></article>`).join('') : '<div class="empty-state">还没有已登记备份。</div>';
  } catch (error) {
    backupTarget.innerHTML = `<div class="empty-state">当前角色无备份管理权限。</div>`;
    document.querySelector('#create-backup').hidden = true;
  }
  document.querySelector('#purge-retention').hidden = !['OWNER', 'ADMIN'].includes(currentUser?.role);
  refreshIcons();
}
...
function showDecision() {
  ...
  const unresolved = currentRun.requirements.filter((item) => ['UNKNOWN', 'NEEDS_REVIEW'].includes(item.status));
  ...
  select.replaceChildren(...unresolved.map((item) => {
    ...
    option.selected = currentRun.decision?.unresolved_requirement_ids?.includes(item.requirement_id) || false;
    return option;
  }));
  const decision = currentRun.decision?.decision || 'HOLD';
  const radio = document.querySelector(`#decision-form input[value="${decision}"]`);
  if (radio) radio.checked = true;
  document.querySelector('#decision-note').value = currentRun.decision?.note || '';
  document.querySelector('#decision-context-content').innerHTML = `<div class="context-metric"><strong>${currentRun.blocker_count}</strong><span>资格 / 废标风险</span></div><div class="context-metric"><strong>${currentRun.unresolved_count}</strong><span>未解决要求</span></div><div class="context-metric"><strong>${currentRun.requirement_count}</strong><span>全部要求项</span></div><p>${currentRun.decision?.note ? escapeHtml(currentRun.decision.note) : '尚未记录人工决定。请先核对高风险项与证据缺口。'}</p>`;
  refreshIcons();
}
...
async function submitDecision(event) {
  ...
  message.textContent = '';
  try {
    currentRun = await request(`/api/runs/${encodeURIComponent(currentRun.run_id)}/decision`, {
...
function renderDetail() {
  document.querySelector('#detail-title').textContent = currentRun.tender_filename;
  document.querySelector('#detail-subtitle').textContent = `${currentRun.run_id.slice(0, 12)} · 版本 ${currentRun.version_number || 1} · ${currentRun.requirement_count} 项要求 · ${currentRun.evidence_assets.length} 份企业证据`;
  document.querySelector('#detail-updated').textContent = `更新于 ${formatDate(currentRun.updated_at)}`;
  const metrics = [
    ['致命风险', currentRun.blocker_count, 'danger', 'shield-alert'],
    ['待复核', currentRun.unresolved_count, 'warning', 'circle-help'],
    ['要求项', currentRun.requirement_count, 'neutral', 'list-checks'],
    ['人工决定', decisionLabel(currentRun.decision?.decision || '未记录'), 'decision', 'clipboard-check'],
  ];
  document.querySelector('#detail-summary').replaceChildren(...metrics.map(([label, value, tone, icon]) => {
    const card = document.createElement('div');
    card.className = `metric ${tone}`;
    card.innerHTML = `<span class="metric-icon"><i data-lucide="${icon}"></i></span><span><span>${label}</span><strong>${escapeHtml(String(value))}</strong></span>`;
    return card;
  }));
  renderSourceFiles();
  document.querySelector('#audit-summary').innerHTML = `<div class="audit-metric"><span>企业证据</span><strong>${currentRun.evidence_assets.length}</strong></div><div class="audit-metric"><span>高风险项</span><strong>${currentRun.blocker_count}</strong></div><div class="audit-metric"><span>待复核项</span><strong>${currentRun.unresolved_count}</strong></div><p class="audit-copy">只有招标原文与企业证据均有页码引用时，要求项才允许判定为 PASS。</p>`;
  const quality = currentRun.scan_quality || {};
  document.querySelector('#quality-summary').innerHTML = `<p class="eyebrow">文本质量</p><p>${quality.total_pages || 0} 页 · ${quality.ocr_required_pages || 0} 页需 OCR · ${quality.ocr_failed_pages || 0} 页 OCR 失败</p><small>${escapeHtml(quality.interpretation || '规则初筛结果，需人工复核。')}</small>`;
  const duplicateWarning = document.querySelector('#duplicate-warning');
  duplicateWarning.hidden = !(currentRun.duplicate_run_ids || []).length;
  duplicateWarning.innerHTML = duplicateWarning.hidden ? '' : `<i data-lucide="copy-check"></i><span>检测到同一招标文件已有 ${currentRun.duplicate_run_ids.length} 个任务：${currentRun.duplicate_run_ids.map((id) => escapeHtml(id.slice(0, 12))).join('、')}。请先确认是否需要重复扫描。</span>`;
  loadAssigneeOptions();
  document.querySelector('#run-tags').value = (currentRun.tags || []).join(', ');
  document.querySelector('#run-favorite').checked = Boolean(currentRun.favorite);
  renderRisks();
  renderMatrix();
  loadCollaboration();
  loadVersionDiff();
...
function renderSourceFiles() {
  ...
  const documents = currentRun?.source_documents || [];
  if (!documents.length) {
    target.innerHTML = '<div class="empty-state">未保存可下载的原始材料索引。</div>';
    return;
  }
  target.innerHTML = documents.map((item) => {
    ...
    const sha = item.sha256 ? ` · SHA-256 ${escapeHtml(item.sha256.slice(0, 12))}…` : '';
    return `<a class="source-file-row" href="/api/runs/${encodeURIComponent(currentRun.run_id)}/files/${encodeURIComponent(item.source_id)}" target="_blank" rel="noopener" download><span class="file-icon"><i data-lucide="${icon}"></i></span><span class="operation-main"><strong>${escapeHtml(item.filename || item.source_id)}</strong><small>${role} · ${escapeHtml(item.file_type || 'file')} · ${Number(item.pages || 0)} 个定位单元${sha}</small></span><i data-lucide="download" aria-hidden="true"></i></a>`;
  }).join('');
  refreshIcons();
}
...
async function loadAssigneeOptions() {
  ...
  try {
    if (!membersCache.length) membersCache = (await request('/api/members')).members;
    const options = membersCache.filter((member) => member.active).map((member) => `<option value="${escapeHtml(member.user_id)}">${escapeHtml(member.username)} · ${escapeHtml(roleLabel(member.role))}</option>`).join('');
    assignee.innerHTML = '<option value="">未分配</option>' + options;
    reviewer.innerHTML = '<option value="">未分配</option>' + options;
    assignee.value = currentRun.assignee_id || '';
    reviewer.value = currentRun.reviewer_id || '';
  } catch (_error) {
    assignee.innerHTML = `<option value="${escapeHtml(currentRun.assignee_id || '')}">${escapeHtml(currentRun.assignee_id || '未分配')}</option>`;
    reviewer.innerHTML = `<option value="${escapeHtml(currentRun.reviewer_id || '')}">${escapeHtml(currentRun.reviewer_id || '未分配')}</option>`;
  }
}
...
async function loadVersionDiff() {
  ...
  target.innerHTML = '<div class="run-skeleton"></div>';
  try {
    const diff = await request(`/api/runs/${encodeURIComponent(currentRun.run_id)}/diff/${encodeURIComponent(currentRun.parent_run_id)}`);
    const items = [['新增', diff.added, 'plus'], ['移除', diff.removed, 'minus'], ['状态变化', diff.changed, 'refresh-ccw']];
    target.innerHTML = items.map(([label, values, icon]) => `<div class="diff-block"><span><i data-lucide="${icon}"></i>${label}</span><strong>${values.length}</strong><small>${values.slice(0, 3).map((item) => escapeHtml((item.after || item).title || '')).join('；') || '无'}</small></div>`).join('');
  } catch (error) { target.innerHTML = `<div class="empty-state error-text">${escapeHtml(error.message)}，版本差异加载失败。</div>`; }
  refreshIcons();
}
...
async function saveRunMetadata(event) {
  ...
  try {
    currentRun = await request(`/api/runs/${encodeURIComponent(currentRun.run_id)}/metadata`, {
...
async function addComment(event) {
  ...
  if (!body) return;
  try {
    await request(`/api/runs/${encodeURIComponent(currentRun.run_id)}/comments`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ body }) });
    input.value = '';
...
async function loadCollaboration() {
  ...
  try {
    const [comments, audit, remediations] = await Promise.all([
      request(`/api/runs/${encodeURIComponent(currentRun.run_id)}/comments`),
      request(`/api/runs/${encodeURIComponent(currentRun.run_id)}/audit`),
      request(`/api/runs/${encodeURIComponent(currentRun.run_id)}/remediations`),
    ]);
    document.querySelector('#comments-list').innerHTML = comments.comments.length ? comments.comments.map((item) => `<article class="activity-item"><strong>${escapeHtml(item.user_id)}</strong><p>${escapeHtml(item.body)}</p><time>${escapeHtml(formatDate(item.created_at))}</time></article>`).join('') : '<div class="empty-state">暂无评论</div>';
    document.querySelector('#audit-events').innerHTML = audit.events.length ? audit.events.slice(0, 30).map((item) => `<article class="activity-item"><strong>${escapeHtml(auditLabel(item.event_type))}</strong><p>${escapeHtml(item.user_id)}</p><time>${escapeHtml(formatDate(item.created_at))}</time></article>`).join('') : '<div class="empty-state">暂无审计记录</div>';
    await loadRemediationOwners();
    renderRemediations(remediations.remediations || []);
  } catch (error) {
    document.querySelector('#comments-list').innerHTML = '<div class="empty-state error-text">协作记录加载失败</div>';
    document.querySelector('#remediations-list').innerHTML = `<div class="empty-state error-text">${escapeHtml(error.message)}，整改项加载失败</div>`;
  }
}
...
async function loadRemediationOwners() {
  ...
  try {
    if (!membersCache.length) membersCache = (await request('/api/members')).members;
    select.innerHTML = '<option value="">未分配</option>' + membersCache.filter((member) => member.active).map((member) => `<option value="${escapeHtml(member.user_id)}">${escapeHtml(member.username)} · ${escapeHtml(roleLabel(member.role))}</option>`).join('');
  } catch (_error) {
    select.innerHTML = '<option value="">未分配</option>';
  }
  const requirementSelect = document.querySelector('#remediation-requirement');
  requirementSelect.innerHTML = '<option value="">不关联具体要求</option>' + (currentRun?.requirements || []).map((item) => `<option value="${escapeHtml(item.requirement_id)}">${escapeHtml(item.requirement_id)} · ${escapeHtml(item.label)}</option>`).join('');
}

function renderRemediations(items) {
  const target = document.querySelector('#remediations-list');
  if (!items.length) {
    target.innerHTML = '<div class="empty-state">暂无整改行动。发现证据缺口后，可在这里分派负责人并跟踪截止日期。</div>';
    return;
  }
  target.innerHTML = items.map((item) => {
    const owner = membersCache.find((member) => member.user_id === item.owner_id);
    ...
    return `<article class="remediation-row ${overdue ? 'is-overdue' : ''}"><div class="remediation-main"><strong>${escapeHtml(item.title)}</strong><small>${escapeHtml(item.requirement_id ? `关联要求 ${item.requirement_id}` : '未关联具体要求')} · ${escapeHtml(owner?.username || item.owner_id || '未分配')} · ${overdue ? '已逾期' : `截止 ${due}`}</small>${item.note ? `<p>${escapeHtml(item.note)}</p>` : ''}</div><div class="remediation-controls"><label class="sr-only" for="remediation-status-${escapeHtml(item.remediation_id)}">整改状态</label><select id="remediation-status-${escapeHtml(item.remediation_id)}" data-remediation-status="${escapeHtml(item.remediation_id)}"><option value="OPEN" ${item.status === 'OPEN' ? 'selected' : ''}>待处理</option><option value="IN_PROGRESS" ${item.status === 'IN_PROGRESS' ? 'selected' : ''}>处理中</option><option value="DONE" ${item.status === 'DONE' ? 'selected' : ''}>已完成</option><option value="CANCELLED" ${item.status === 'CANCELLED' ? 'selected' : ''}>已取消</option></select></div></article>`;
  }).join('');
}
...
async function createRemediation(event) {
  ...
  setButtonLoading(button, true, '创建中');
  try {
    await request(`/api/runs/${encodeURIComponent(currentRun.run_id)}/remediations`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
...
function renderRisks() {
  const risks = currentRun.requirements
  ...
  if (!risks.length) {
    target.innerHTML = '<div class="empty-state success-text">当前没有需要优先处理的高风险项。</div>';
    return;
  }
...
function renderMatrix() {
  const categories = ['ALL', ...new Set(currentRun.requirements.map((item) => item.category))];
  ...
  filters.replaceChildren(...categories.map((category) => {
    ...
    button.addEventListener('click', () => { activeCategory = category; matrixPage = 1; renderMatrix(); });
    return button;
  }));
  const filtered = currentRun.requirements.filter((item) => {
  ...
  matrixPage = Math.min(matrixPage, pageCount);
  ...
  if (!items.length) target.innerHTML = '<div class="empty-state">没有符合当前条件的要求项。</div>';
...
function renderRiskCard(item) {
  ...
  article.innerHTML = `${requirementHeading(item)}<p class="requirement-title">${escapeHtml(item.title)}</p>${citationMarkup(item)}${explanationMarkup(item)}${requirementFooter(item)}`;
  return article;
}
...
function renderRequirementRow(item) {
  ...
  details.innerHTML = `<summary class="requirement-summary"><span class="requirement-name"><strong>${escapeHtml(item.label)}</strong><small>${escapeHtml(item.category)} · ${escapeHtml(item.requirement_id)}</small></span><span class="source-page">${escapeHtml(locatorLabel(source))}</span><span class="status ${escapeHtml(item.status)}">${escapeHtml(statusLabel(item.status))}</span><span class="expand-icon"><i data-lucide="chevron-down"></i></span></summary><div class="requirement-detail"><p class="requirement-title">${escapeHtml(item.title)}</p>${citationMarkup(item)}${explanationMarkup(item)}${requirementFooter(item)}</div>`;
  return details;
}

function requirementHeading(item) {
  return `<div class="requirement-top"><div><span class="category-label">${escapeHtml(item.category)}</span><h3>${escapeHtml(item.label)}</h3></div><span class="status ${escapeHtml(item.status)}">${escapeHtml(statusLabel(item.status))}</span></div>`;
}
...
function citationMarkup(item) {
  ...
  const evidenceMarkup = evidence.length
    ? evidence.map((entry) => `<strong>${escapeHtml(entry.filename)} · ${escapeHtml(locatorLabel(entry))}</strong><p>${escapeHtml(entry.quote || '已定位')}</p>`).join('')
    : '<strong class="muted">未匹配，需人工复核</strong>';
  return `<div class="citation-grid"><div class="citation-block"><span>招标原文</span><strong>${escapeHtml(locatorLabel(source))}</strong><p>${escapeHtml(source.quote || '未提取到可引用原文')}</p></div><div class="citation-block"><span>企业证据</span>${evidenceMarkup}</div></div>`;
}
...
function explanationMarkup(item) {
  ...
  return `<div class="explanation-grid"><div><span>证据缺口</span><strong>${escapeHtml(gap)}</strong></div><div><span>风险影响</span><strong>${escapeHtml(impact)}</strong></div><div><span>建议动作</span><strong>${escapeHtml(action)}</strong></div></div>`;
}

function requirementFooter(item) {
  const actions = item.status === 'PASS' || item.status === 'FAIL'
    ? `<button class="mini-button" data-review="CONFIRM" data-id="${escapeHtml(item.requirement_id)}">确认结论</button><button class="mini-button" data-review="REJECT" data-id="${escapeHtml(item.requirement_id)}">驳回结论</button>`
    : `<button class="mini-button" data-review="REQUEST_EVIDENCE" data-id="${escapeHtml(item.requirement_id)}">请求证据</button><button class="mini-button" data-review="CONFIRM" data-id="${escapeHtml(item.requirement_id)}">保留复核</button>`;
  return `<div class="requirement-footer"><span>${escapeHtml(item.detection_method || 'deterministic')} · ${escapeHtml(item.criticality || 'REVIEW')}</span><div class="action-buttons">${actions}<button class="mini-button" data-accuracy="RELEVANT" data-id="${escapeHtml(item.requirement_id)}" data-category="${escapeHtml(item.category)}">确认有效</button><button class="mini-button" data-accuracy="NOT_RELEVANT" data-id="${escapeHtml(item.requirement_id)}" data-category="${escapeHtml(item.category)}">标记误报</button></div></div>`;
}
...
function renderPagination(total, pageCount) {
  ...
  const start = (matrixPage - 1) * PAGE_SIZE + 1;
  const end = Math.min(matrixPage * PAGE_SIZE, total);
  target.innerHTML = `<span class="pagination-info">显示 ${start}-${end}，共 ${total} 项</span><span class="pagination-actions"><button class="page-button" type="button" data-page="prev" ${matrixPage === 1 ? 'disabled' : ''}><i data-lucide="arrow-left"></i><span>上一页</span></button><button class="page-button" type="button" data-page="next" ${matrixPage === pageCount ? 'disabled' : ''}><span>下一页</span><i data-lucide="arrow-right"></i></button></span>`;
  target.querySelector('[data-page="prev"]').addEventListener('click', () => { matrixPage -= 1; renderMatrix(); scrollMatrixIntoView(); });
  target.querySelector('[data-page="next"]').addEventListener('click', () => { matrixPage += 1; renderMatrix(); scrollMatrixIntoView(); });
}
...
async function loadAccuracySummary() {
  ...
  try {
    ...
    target.innerHTML = metrics.categories.length ? metrics.categories.slice(0, 4).map((item) => {
      const status = item.measurement_status === 'MEASURABLE' ? '可计量' : '证据不足';
      const coverage = item.coverage == null ? '—' : `${Math.round(item.coverage * 100)}%`;
      return `<div><span class="rule-icon ${item.measurement_status === 'MEASURABLE' ? 'safe' : 'warning'}"><i data-lucide="chart-no-axes-combined"></i></span><span><strong>${escapeHtml(item.category)} · ${status}</strong><small>观察精确率 ${item.precision ?? '—'} · 召回率 ${item.recall ?? '—'} · 覆盖 ${coverage} · 样本 ${item.sample_size} · 完整复核 ${item.review_population_complete ? '是' : '否'}</small></span></div>`;
    }).join('') : '<div class="empty-state">尚无人工反馈样本</div>';
    refreshIcons();
  } catch (error) { target.innerHTML = '<div class="empty-state error-text">指标加载失败</div>'; }
}

async function reviewRequirement(requirementId, decision, button) {
  button.disabled = true;
  try {
    currentRun = await request(`/api/runs/${encodeURIComponent(currentRun.run_id)}/review`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
