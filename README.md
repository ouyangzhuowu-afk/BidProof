...
- 当前公网试点数据在 Render 新加坡。面向国内政企的境内节点与等保测评是独立交付线，不在本 Render 部署内完成。可选字段加密：`BIDPROOF_FIELD_ENCRYPTION_KEY`（客户自带口令，BYOK）。ISO 27001 认证未启动，差距清单见 `docs/iso27001-gap.md`。
